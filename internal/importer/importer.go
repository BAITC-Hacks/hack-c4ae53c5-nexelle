...
func LoadFiles(ctx context.Context, skillsReader, eventsReader, employeesReader, historyReader io.Reader) (domain.Dataset, ValidationReport, error) {
	...
	for _, event := range dataset.Events {
		for _, prerequisite := range event.PrerequisiteEvents {
			if _, exists := dataset.Events[prerequisite]; !exists || prerequisite == event.EventID {
				report.add("events.json", 0, "prerequisite_events", "unknown or self-referencing event "+prerequisite)
			}
		}
	}
...
func readJSON[T any](reader io.Reader) (T, error) {
	...
	if err := decoder.Decode(&trailing); !errors.Is(err, io.EOF) {
		return result, fmt.Errorf("expected exactly one JSON document")
	}
	return result, nil
}
...
func loadEvents(dataset *domain.Dataset, events []domain.Event, report *ValidationReport) {
	for _, event := range events {
		...
		if event.TargetRoles == nil || event.TargetGrades == nil || event.DevelopsSkills == nil || event.Prerequisites == nil || event.UpcomingSessions == nil {
			report.add("events.json", 0, "event", "target_roles, target_grades, develops_skills, prerequisites and upcoming_sessions are required")
		}
		for _, role := range event.TargetRoles {
			known := false
			for _, profile := range dataset.RoleProfiles {
				if profile.Role == role {
					known = true
				}
			}
			if !known {
				report.add("events.json", 0, "target_roles", "unknown role "+role)
			}
		}
		for _, grade := range event.TargetGrades {
			if !isGrade(grade) {
				report.add("events.json", 0, "target_grades", "unknown grade "+grade)
			}
		}
		...
		for _, gain := range event.DevelopsSkills {
			if seenSkills[gain.SkillID] {
				report.add("events.json", 0, "develops_skills", "duplicate skill "+gain.SkillID)
			}
			seenSkills[gain.SkillID] = true
