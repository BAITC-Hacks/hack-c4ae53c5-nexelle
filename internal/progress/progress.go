...
func Calculate(dataset domain.Dataset, employee domain.Employee, activities []domain.ActivityRecord, cutoffDate time.Time) (domain.CareerProgress, error) {
	...
	for _, activity := range sortedActivities {
		...
		for _, gain := range event.DevelopsSkills {
			...
			effectiveSkills[gain.SkillID] = currentLevel + EffectiveGain(currentLevel, gain)
		}
	}
...
// EffectiveGain is shared by scoring and completion, and never lowers a skill.
func EffectiveGain(current int, gain domain.EventSkillGain) int {
	if current >= gain.MaxLevel || current >= domain.AbsoluteMaxSkillLevel {
		return 0
	}
	return max(0, min(min(current+gain.Gain, gain.MaxLevel), domain.AbsoluteMaxSkillLevel)-current)
}
