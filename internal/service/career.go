...
func NewCareerService(datasetStore store.DatasetStore, cutoffDate time.Time) (*CareerService, error) {
	...
	return &CareerService{store: datasetStore, cutoffDate: cutoffDate, explainer: explanation.New(nil)}, nil
}

// SetExplainer configures the optional text provider before serving requests.
func (s *CareerService) SetExplainer(adapter *explanation.Adapter) { s.explainer = adapter }
...
func (s *CareerService) GetProfile(ctx context.Context, employeeID string) (domain.ProfileResponse, error) {
	...
	recommendations = s.explainer.Explain(ctx, recommendations, explanation.Locale(ctx))
	return domain.ProfileResponse{
		CutoffDate:      s.cutoffDate.Format(time.DateOnly),
...
func (s *CareerService) CompleteEvent(ctx context.Context, employeeID, eventID string) (domain.CompletionResponse, error) {
	...
	if !recommendation.MeetsPrerequisites(event, careerProgress.EffectiveSkills, completed) {
		return domain.CompletionResponse{}, ErrPrerequisitesNotMet
	}
...
func (s *CareerService) GetHRAnalytics(ctx context.Context) (domain.HRAnalytics, error) {
	...
	for _, employeeID := range employeeIDs {
		...
		for _, activity := range activities {
			if !activity.Date.After(s.cutoffDate) && activity.Status == domain.ActivityInProgress {
				inDevelopment++
				break
			}
		}
	...
	participation := participationByEvent(dataset, s.cutoffDate)
	return domain.HRAnalytics{
		TotalEmployees:                 len(dataset.Employees),
		EmployeesInDevelopment:         inDevelopment,
...
func (s *CareerService) profileData(ctx context.Context, employeeID string) (domain.Dataset, domain.Employee, []domain.ActivityRecord, error) {
	...
	employee, exists := dataset.Employees[employeeID]
	if !exists {
		return domain.Dataset{}, domain.Employee{}, nil, store.ErrEmployeeNotFound
	}
	activities := make([]domain.ActivityRecord, 0)
	for _, activity := range dataset.ActivitiesByEmployee[employeeID] {
		if !activity.Date.After(s.cutoffDate) {
...
func participationByEvent(dataset domain.Dataset, cutoff time.Time) []domain.EventParticipation {
