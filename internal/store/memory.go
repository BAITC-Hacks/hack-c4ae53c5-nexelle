...
func (s *MemoryStore) GetDataset(ctx context.Context) (domain.Dataset, error) {
	...
	return copyDataset(*s.dataset), nil
}
...
func copyEvent(event domain.Event) domain.Event {
	...
	result.PrerequisiteEvents = append([]string(nil), event.PrerequisiteEvents...)
...
func copyDataset(source domain.Dataset) domain.Dataset {
	result := domain.NewDataset()
	result.ProficiencyScale = maps.Clone(source.ProficiencyScale)
	result.Skills = maps.Clone(source.Skills)
	for key, profile := range source.RoleProfiles {
		profile.RequiredSkills = maps.Clone(profile.RequiredSkills)
		profile.CriticalSkills = append([]string(nil), profile.CriticalSkills...)
		result.RoleProfiles[key] = profile
	}
	for key, employee := range source.Employees {
		result.Employees[key] = copyEmployee(employee)
	}
	for key, event := range source.Events {
		result.Events[key] = copyEvent(event)
	}
	for key, activity := range source.ActivityRecords {
		result.ActivityRecords[key] = copyActivity(activity)
	}
	for key, activities := range source.ActivitiesByEmployee {
		result.ActivitiesByEmployee[key] = make([]domain.ActivityRecord, len(activities))
		for index, activity := range activities {
			result.ActivitiesByEmployee[key][index] = copyActivity(activity)
		}
	}
	return result
}

func copyActivity(activity domain.ActivityRecord) domain.ActivityRecord {
	result := activity
	...
	if activity.FeedbackRating != nil {
		value := *activity.FeedbackRating
		result.FeedbackRating = &value
	}
	return result
}
