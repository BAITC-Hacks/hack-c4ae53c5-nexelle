...
export default function App() {
  ...
  const loadEmployee = async (signal) => {
    const version = ++requestVersion.current
    setLoading(true)
    setError(false)
    try {
      const result = await api.employee(locale, signal)
  ...
  const loadHR = async (signal) => {
    const version = ++requestVersion.current
    setLoading(true)
    setError(false)
    try {
      const result = await api.hr(locale, signal)
      if (version !== requestVersion.current || signal?.aborted) return
      setHrData(result)
    } catch {
      if (!signal?.aborted && version === requestVersion.current) setError(true)
  ...
  useEffect(() => {
    ...
    if (role === 'employee') loadEmployee(controller.signal)
    else loadHR(controller.signal)
  ...
  const handleComplete = async (eventId) => {
    ...
    setToast('')
    try {
      const result = await api.complete(eventId, locale)
