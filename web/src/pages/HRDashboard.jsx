...
export default function HRDashboard({ data, t }) {
  ...
  const completedRate = participationTotal ? Math.round((participation.completed / participationTotal) * 100) : 0
  const highestGap = Math.max(1, ...data.skill_gaps.map((item) => item.employees))
